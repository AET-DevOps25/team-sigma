...
export class CdkStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const apiGatewayImage = ecs.ContainerImage.fromAsset(path.join(REPO_ROOT, 'server', 'api-gateway'));

    const apiGatewayService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, 'ApiGatewayService', {
      cluster,
      desiredCount: 1,
    ...
    const clientImage = ecs.ContainerImage.fromAsset(path.join(REPO_ROOT, 'client'));

    const clientService = new ecs_patterns.ApplicationLoadBalancedFargateService(this, 'ClientService', {
      cluster,
      desiredCount: 1,
      publicLoadBalancer: true,
      memoryLimitMiB: 512,
    ...
    createInternalService(
      'DocumentService',
      ecs.ContainerImage.fromAsset(path.join(REPO_ROOT, 'server', 'document-service')),
      8080,
      {
    ...
    createInternalService(
      'ChatService',
      ecs.ContainerImage.fromAsset(path.join(REPO_ROOT, 'server', 'chat-service')),
      8082,
      {
    ...
    createInternalService(
      'LectureService',
      ecs.ContainerImage.fromAsset(path.join(REPO_ROOT, 'server', 'lecture-service')),
      8083,
      {
    ...
    createInternalService(
      'Weaviate',
      ecs.ContainerImage.fromRegistry('cr.weaviate.io/semitechnologies/weaviate:stable-v1.31-9900730'),
      8080,
      {
    ...
    createInternalService(
      'Minio',
      ecs.ContainerImage.fromRegistry('minio/minio:RELEASE.2025-05-24T17-08-30Z'),
      9000,
      {
